...
namespace PortedPrediction.All.HealthPrediction
{
    public class HealthPrediction
    {
        ...
        {                                                                                           //BUG !Orbwalker.IsAutoAttacking(args.SData.Name)
            if (sender.Team != ObjectManager.Player.Team || !sender.IsValidTarget(3000, false) || !Orbwalker.IsAutoAttacking || !(args.Target is Obj_AI_Base))
            {
                return;
            }
        ...
        public static float GetHealthPrediction(Obj_AI_Base unit, int time, int delay = 70)
        {
            ...
            foreach (var attack in ActiveAttacks.Values)
            {
                    ...
                    attack.Target.IsValidTarget(float.MaxValue, false) && attack.Target.NetworkId == unit.NetworkId)
                {
                    ...
                    if (/*Utils.GameTimeTickCount < landTime - delay &&*/ landTime < Core.GameTickCount + time)
                    {
        ...
        public static float LaneClearHealthPrediction(Obj_AI_Base unit, int time, int delay = 70)
        {
            ...
            {
                var n = 0;
                if (Core.GameTickCount - 100 <= attack.StartTick + attack.AnimationTime &&
                    ...
                    attack.Source.IsValidTarget(float.MaxValue, false) && attack.Target.NetworkId == unit.NetworkId)
                {
                    ...
                    var toT = Core.GameTickCount + time;

                    while (fromT < toT)
                    {
                        if (fromT >= Core.GameTickCount &&
