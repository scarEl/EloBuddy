...
using PortedPrediction.All;
using System.Drawing;

namespace PortedPrediction.All.Prediction
{
    ...
    public static class Prediction
    {
        ...
        internal static PredictionOutput GetPrediction(PredictionInput input, bool ft, bool checkCollision)
        {
            ...
            if (result == null)
            {
                result = GetPositionOnPath(input, PCommon.PCommon.GetWaypoints(input.Unit), input.Unit.MoveSpeed);
            }

            /*if (input.Unit is Obj_AI_Base && input.Radius > 1 && result.Hitchance <= HitChance.VeryHigh)
            {
                ...
                if (!wallPoint.IsZero)
                {
                    ...
                    result.CastPosition = wallPoint.Extend(result.CastPosition,wallPoint);
                }
            }*/
        ...
        {
            if (!(input.Unit is Obj_AI_Base) || input.Radius == 1)
            {
            ...
            {
                //PCommon.debug("CAN'T MOVE SPELLS");
                result.Hitchance = HitChance.VeryHigh;
            ...
            {
                //PCommon.debug("PRED M: NEW VISABLE");
                result.Hitchance = HitChance.Medium;
                return result;
            }

            // PREPARE MATH ///////////////////////////////////////////////////////////////////////////////////
            var path = PCommon.PCommon.GetWaypoints(input.Unit);
            ...
            {
                //PCommon.debug("PRED M: SHORT CLICK DETECTION");
                result.Hitchance = HitChance.Medium;
                return result;
            }

            if (input.Unit.IsRooted)
            {
                if (totalDelay > 0.6)
                {
                    PCommon.PCommon.debug("PRED M: winding block");
                    result.Hitchance = HitChance.Medium;
                }
                else
                {
                    PCommon.PCommon.debug("PRED VH: winding accept");
                    result.Hitchance = HitChance.VeryHigh;
                }
                return result;
            }
            ...
            {
                PCommon.PCommon.debug("PRED VH: After CC detection " + totalDelay);
                result.Hitchance = HitChance.VeryHigh;
                return result;
            }
            ...
            if (distanceUnitToWaypoint > 0)
            {
                ...
                if (getAngle < 20 || getAngle > 160 || ((getAngle > 150 || getAngle < 30) && PCommon.PCommon.IsMovingInSameDirection(ObjectManager.Player, input.Unit)))
                {
                    //PCommon.debug("PRED VH: ANGLE " + getAngle);
                    result.Hitchance = HitChance.VeryHigh;
                    return result;
                }
                ...
                var points = PCommon.PCommon.CirclePoints(10, 200, input.Unit.Position).Where(x => x.IsWall());
                ...
                if (points.Count() > 2)
                {
                    ...
                    if (runOutWall)
                    {
                        PCommon.PCommon.debug("PRED VH: RUN OUT WALL");
                        result.Hitchance = HitChance.VeryHigh;
                        return result;
                    }
                }
                ...
                {
                    // LONG TIME ///////////////////////////////////////////////////////////////////////////////////
                    PCommon.PCommon.debug("PRED VH: LONG TIME");
                    result.Hitchance = HitChance.VeryHigh;
                    return result;
                }
            }
            ...
            if (PCommon.PCommon.GetWaypoints(input.Unit).Count == 1)
            {
                if (UnitTracker.GetLastAutoAttackTime(input.Unit) < 0.1d && totalDelay < 0.7)
                {
                    PCommon.PCommon.debug("PRED VH: AA try");
                    result.Hitchance = HitChance.VeryHigh;
                    return result;
                }
                if (input.Unit.IsRooted)
                ...
                {
                    PCommon.PCommon.debug("PRED: STOP HIGH");
                    result.Hitchance = HitChance.High;
                    return result;
                }
                else
                {
                    PCommon.PCommon.debug("PRED VH: STOP LOGIC");
                    result.Hitchance = HitChance.VeryHigh;
                    return result;
                }
            }
            ...
            {
                PCommon.PCommon.debug("PRED VH: SPECIAL CASES NEAR");
                result.Hitchance = HitChance.VeryHigh;
                return result;
            }
            else if (input.Unit.MoveSpeed < 250)
            {
                PCommon.PCommon.debug("PRED VH: SPECIAL CASES SLOW");
                result.Hitchance = HitChance.VeryHigh;
                return result;
            }
            else if (distanceFromToWaypoint < 250)
            {
                PCommon.PCommon.debug("PRED VH: SPECIAL CASES ON WAY");
                result.Hitchance = HitChance.VeryHigh;
                return result;
            }
            ...
            {
                PCommon.PCommon.debug("PRED VH: LONG CLICK DETECTION");
                result.Hitchance = HitChance.VeryHigh;
                return result;
            }
            ...
            {
                PCommon.PCommon.debug("PRED VH: NEW PATH");
                result.Hitchance = HitChance.VeryHigh;
            }
        ...
        internal static PredictionOutput GetDashingPrediction(PredictionInput input)
        {
            ...
            //Normal dashes.
            if (input.Unit.IsDashing() /*!dashData.IsBlink*/)
            {
                //Mid air:
        ...
        {
            var count = 30;
            var points = PCommon.PCommon.CirclePoints(count, range, from);
        ...
        internal static PredictionOutput GetPositionOnPath(PredictionInput input, List<Vector2> path, float speed = -1)
        {
            ...
            if (path.Count <= 1 || (input.Unit.IsRooted && !input.Unit.IsDashing()))
            {
                return new PredictionOutput
                {
                    Input = input,
    ...
    internal static class AoePrediction
    {
            ...
            public static PredictionOutput GetPrediction(PredictionInput input)
            {
                ...
                while (posibleTargets.Count > 1)
                {
                        ...
                        input.Range * input.Range)
                    {
                        return new PredictionOutput
                        {
                            AoeTargetsHit = posibleTargets.Select(h => (Obj_AI_Base)h.Unit).ToList(),
        ...
        public static class Line
        {
            ...
            internal static Vector2[] GetCandidates(Vector2 from, Vector2 to, float radius, float range)
            {
                ...
                var intersections = new Geometry.Polygon.Circle(middlePoint, radius, 20);
                //(from, middlePoint, radius, from.Distance(middlePoint));

                //Geometry.Polygon.Circle intersections = new Geometry.Polygon.Circle(middlePoint, radius,20);

                if (intersections.Radius > 1) //.Length > 1)
                {
                    var c1 = intersections.Points[0]; // [0]
                    var c2 = intersections.Points[1]; // [1]
    ...
    public static class Collision
    {
        ...
        private static bool MinionIsDead(PredictionInput input, Obj_AI_Base minion, float distance)
        {
            ...
            if (HealthPrediction.HealthPrediction.LaneClearHealthPrediction(minion, convert, 0) <= 0)
            {
                return true;
            }
            else
                ...
                foreach (var objectType in input.CollisionObjects)
                {
                    switch (objectType)
                    {
                        case CollisionableObjects.Minions:
                            foreach (var minion in Cache.Cache.GetMinions(input.From, Math.Min(input.Range + input.Radius + 100, 2000),EntityManager.UnitTeam.Enemy))
                            {
    ...
    internal static class UnitTracker
    {
        ...
        private static List<Obj_AI_Base> Champion = new List<Obj_AI_Base>();
        ...
        static UnitTracker()
        {
            ...
            {
                Champion.Add(hero);
                UnitTrackerInfoList.Add(new UnitTrackerInfo() { NetworkId = hero.NetworkId, AaTick = Core.GameTickCount, StopMoveTick = Core.GameTickCount, NewPathTick = Core.GameTickCount, SpecialSpellFinishTick = Core.GameTickCount, LastInvisableTick = Core.GameTickCount });
            }
        ...
        private static void Obj_AI_Base_OnEnterLocalVisiblityClient(AttackableUnit sender, EventArgs args)
        {
            if (sender is Obj_AI_Base)
                UnitTrackerInfoList.Find(x => x.NetworkId == sender.NetworkId).LastInvisableTick = Core.GameTickCount;
        }
