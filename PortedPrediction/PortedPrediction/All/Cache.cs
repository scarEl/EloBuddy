...
namespace PortedPrediction.All.Cache
{
    public class Cache
    {
        ...
        public static List<Obj_AI_Base> GetMinions(Vector3 from, float range /*= float.MaxValue*/, EntityManager.UnitTeam team)
        {
            ...
            else if (team == EntityManager.UnitTeam.Ally)
            {

                return MinionsListAlly.FindAll(minion => CanReturn(minion, from, range));
            }
            else if (team == EntityManager.UnitTeam.Both)
            {
