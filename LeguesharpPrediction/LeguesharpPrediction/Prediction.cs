...
namespace LeaguesharpPrediction
{
    ...
    public class PredictionOutput
    {
        ...
        public Obj_AI_Base[] CollisionObjects2
        {
    ...
    public static class Prediction
    {
        ...
        private static Menu CommonMenu;
        ...
        private static void Initialize()
        {
            Loading.OnLoadingComplete += eventargs =>
            {
                ...
                Menu.Add("PredictionSelected",
                    new ComboBox("Prediction Selected:", new List<string> {"SDK Prediction"}, 0));
                EloBuddy.SDK.Prediction.Manager.Suscribe("Laguesharp Prediction", GetLPrediction);
        ...
        private static EloBuddy.SDK.Prediction.Manager.PredictionOutput GetLPrediction(EloBuddy.SDK.Prediction.Manager.PredictionInput input)
        {
            if (input == null || input.Target == null)
            {
                throw new ArgumentNullException("input");
            }
            var sdkInput = new EloBuddy.SDK.Prediction.Position.PredictionData(TransformToPredictionType(input.Type), (int)input.Range, (int)input.Radius, (int)input.Radius, (int)(1000 * input.Delay), (int)input.Speed,
                TransformToCollisionCount(input.CollisionTypes), input.From);
            var sdkOutput = GetPrediction(input.Target, sdkInput.Delay);
            var output = new EloBuddy.SDK.Prediction.Manager.PredictionOutput(input)
            {
            ...
            private static EloBuddy.SDK.Prediction.Position.PredictionData.PredictionType TransformToPredictionType(SkillShotType type)
        {
            switch (type)
            {
                case SkillShotType.Linear:
                    return EloBuddy.SDK.Prediction.Position.PredictionData.PredictionType.Linear;
                case SkillShotType.Circular:
                    return EloBuddy.SDK.Prediction.Position.PredictionData.PredictionType.Circular;
                case SkillShotType.Cone:
                    return EloBuddy.SDK.Prediction.Position.PredictionData.PredictionType.Cone;
            }
            return EloBuddy.SDK.Prediction.Position.PredictionData.PredictionType.Circular;
        }

        private static SkillShotType TransformToSkillShotType(EloBuddy.SDK.Prediction.Position.PredictionData.PredictionType type)
        {
            switch (type)
            {
                case EloBuddy.SDK.Prediction.Position.PredictionData.PredictionType.Linear:
                    return SkillShotType.Linear;
                case EloBuddy.SDK.Prediction.Position.PredictionData.PredictionType.Circular:
                    return SkillShotType.Circular;
                case EloBuddy.SDK.Prediction.Position.PredictionData.PredictionType.Cone:
        ...
        internal static PredictionOutput GetPrediction(PredictionInput input, bool ft, bool checkCollision)
        {
            ...
            else
            {
                ...
                {
                    input.Range = input.Range * Menu["skillshotRangeAdjustment"].Cast<Slider>().CurrentValue / 100f;
                }
            }
        ...
        public static PredictionOutput GetStandardPrediction(PredictionInput input)
        {
            ...
            var result = GetPositionOnPath(input, new List<Vector2>(), speed);
...
internal static class AoePrediction
    {
        ...
        public static class Line
        {
            ...
            internal static Vector2[] GetCandidates(Vector2 from, Vector2 to, float radius, float range)
            {
                ...
                /*var intersections = new Geometry.Polygon.Circle(
                    from,
                    middlePoint,
                    radius,
                    from.Distance(middlePoint));*/

                if (intersections.Radius > 1)
                {                          //Points might be wrong. *added it
                    var c1 = intersections.Points[0];
                    var c2 = intersections.Points[1];
    ...
    public static class Collision
    {
                ...
                foreach (var objectType in input.CollisionObjects)
                {
                    switch (objectType)
                    {
                        ...
                        case CollisionableObjects.YasuoWall:
                            ...
                            if (Core.GameTickCount/*.TickCount*/ - _wallCastT > 4000)
                            {
                                break;
                            }
                            ...
                            {
                                var t = Core.GameTickCount//.TickCount
        ...
        private static void AIHeroClient_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            {
                _wallCastT = Core.GameTickCount;//.TickCount;
    ...
    internal class StoredPath
    {
        ...
        public double Time
        {
            get
            {
                return (Core.GameTickCount/*.TickCount*/ - this.Tick) / 1000d;
            }
        }
    ...
    internal static class PathTracker
    {
        ...
        private static void AIHeroClient_OnNewPath(Obj_AI_Base sender, GameObjectNewPathEventArgs args)
        {
            ...
            var newPath = new StoredPath { Tick = Core.GameTickCount/*.TickCount*/, Path = args.Path.ToList().To2D() };
