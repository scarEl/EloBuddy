...
namespace Polar_Bear
{
    internal class Combo
    {
        ...
        public static void Execute2()
        {
            ...
            {
                if (Spells.Hydra.IsOwned() && Spells.Hydra.IsReady() && target.IsInRange(target, Spells.R.Range))
                {
                    Spells.Hydra.Cast();
                }
                if (Spells.Hydra2.IsOwned() && Spells.Hydra2.IsReady() && target.IsInRange(target, Spells.R.Range))
                {
                    Spells.Hydra2.Cast();
                }
                if (Spells.TitanicHydra.IsOwned() && Spells.TitanicHydra.IsReady() && target.IsInRange(target, Spells.R.Range))
                {
                    Spells.TitanicHydra.Cast();
                }
                if (Spells.Tiamat.IsOwned() && Spells.Tiamat.IsReady() && target.IsInRange(target, Spells.R.Range))
                {
                    Spells.Tiamat.Cast();
                }
                if (Spells.TiamatMelee.IsOwned() && Spells.TiamatMelee.IsReady() && target.IsInRange(target, Spells.R.Range))
                {
